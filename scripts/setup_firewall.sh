...
# 2. Proteksi DNS UDP Flood (High Performance: 1.000.000 QPS Global, 1.500 QPS per IP)
  ...
  # DNS Flood Protection (UDP Port 53) - Ditingkatkan untuk ISP Scale & NAT Topology
    # Mengizinkan hingga 20.000 QPS per IP untuk mengakomodasi ribuan user dibalik satu Public IP (NAT)
    iptables -A INPUT -p udp --dport 53 -m hashlimit --hashlimit-name dns_flood --hashlimit-upto 20000/sec --hashlimit-burst 10000 --hashlimit-mode srcip --hashlimit-htable-expire 120000 -j ACCEPT
   ...
   ip6tables -A INPUT -p udp --dport 53 -m hashlimit --hashlimit-name dns6_flood --hashlimit-upto 20000/sec --hashlimit-burst 10000 --hashlimit-mode srcip --hashlimit-htable-expire 120000 -j ACCEPT 2>/dev/null || true
