...
def get_ssh_config():
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT key, value FROM settings WHERE key IN ('ssh_p_ip', 'ssh_p_port', 'ssh_p_user', 'ssh_p_pass')")
        rows = cur.fetchall()
        conn.close()
...
def run_ssh(cmd):
    cfg = get_ssh_config()
    ip = cfg.get("ssh_p_ip")
    port = cfg.get("ssh_p_port") or "22"
    user = cfg.get("ssh_p_user") or "root"
    pw = cfg.get("ssh_p_pass")
...
def collect():
    init_db()
    print("Starting REAL-TIME system & traffic collector (5s) for Primary...")
    ...
    while True:
        try:
            ...
            if len(lines) >= 7:
                ...
                print(f"Malformed output from Primary (lines={len(lines)})")
                time.sleep(5)
                continue
            ...
            if last_total > 0 and dt > 0:
                if current_total >= last_total:
                    ...
                    # Log rotation detected - check if log was just truncated
                    # If current_total is small, it's a new log
                    qps = current_total / dt
                
                # Filter abnormal spikes (Smoothing)
                if qps > 10000: qps = 0.0 
            else:
                qps = 0.0
